...
import { ICategory } from "@/pages/admin/category";
...
const AdminCategoryView = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/category/" + router.query.id)
      .then((res) => {
        setData(res.data);
      })
      .catch(() => {
  ...
  return (
    <div className="admin__view-page flex h-full w-full bg-[#EFF9F5]">
      ...
      <div className="admin__category--view flex items-center justify-center mx-10 md:mx-20 w-full mt-14 md:mt-20">
        <form className="view__recipe flex flex-col items-start bottom-10 shadow-[#889FA5] shadow-spread py-8 px-10 rounded-10 w-full h-full">
          <h1 className="text-32 font-semibold text-start mb-4">
            View Category
          </h1>
